...
void WaveletNoise(float2 UV, float Angle, float Phase, float SeedScale, float ScaleFactor, out float Out)
{
	float d = 0.0;
	float scale = SeedScale;
	float mag = 0.;
	for (float i = 0.; i < 4.; i++)
	{
		d += GetWavelet(UV, Angle, Phase, scale);
		UV = mul(UV, float2x2(.54, -.84, .84, .54)) + i;
		mag += 1. / scale;
		scale *= ScaleFactor;
	}
	d /= mag;

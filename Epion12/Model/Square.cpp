...
namespace epion::Model
{
	...
	bool Square::Initialize(com_ptr<ID3DBlob>& vs_blob,	com_ptr<ID3DBlob>& ps_blob)
	{
		...
		auto a = sizeof(SquareVertex);
		auto b=sizeof(SquareVertex) * 4;
	...
	{ "TEXCOORD",0,DXGI_FORMAT_R32G32_FLOAT,0,D3D12_APPEND_ALIGNED_ELEMENT,D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA,0 },
		...
		m_pipeline_desc.VS.pShaderBytecode = vs_blob.Get()->GetBufferPointer();
		m_pipeline_desc.VS.BytecodeLength = vs_blob.Get()->GetBufferSize();
		...
		m_pipeline_desc.InputLayout.pInputElementDescs = inputLayout;//レイアウト先頭アドレス
		m_pipeline_desc.InputLayout.NumElements = _countof(inputLayout);//レイアウト配列数
		...
		//rootSignatureDesc.pParameters = &root_param;//ルートパラメータの先頭アドレス
